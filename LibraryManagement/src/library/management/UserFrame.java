    ...
    public UserFrame(int readerId) {
        ...
        btnConfirm.addActionListener(e -> {
            try {
                ...
                String status = "Đang mượn"; // Mặc định
        ...
        Runnable loadLoans = () -> {
            ...
            try (Connection conn = ConnectDB.getConnection()) {
                String sql = "SELECT loanId, bookId, loanDate, bookReturnAppointmentDate, status FROM loan WHERE status = 'Đang mượn' AND readerID = ?";
                PreparedStatement stmt = conn.prepareStatement(sql);
            ...
            try {
                ...
                try (Connection conn = ConnectDB.getConnection()) {
                    ...
                    if (rs.next()) {
                        ...
                        if (!status.equalsIgnoreCase("Đang mượn")) {
